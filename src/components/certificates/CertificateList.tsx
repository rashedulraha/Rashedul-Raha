...
export default function CertificateList({
  ...
  const isLoading = !initialCertificates || initialCertificates.length === 0;
  ...
  }, [initialCertificates, searchQuery]);

  const webDevCertificates = filteredCertificates.filter(
    (c) => c.category === "Web Development Journey",
  );
  const otherCertificates = filteredCertificates.filter(
    (c) => c.category === "Other Achievements",
  );
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="flex flex-col rounded-2xl bg-card border border-border overflow-hidden card-premium shadow-sm p-5 space-y-4"
            >
                ...
                {webDevCertificates.map((cert) => (
                  ...
                  >
                    <Link
                      ...
                      className="block relative h-40 w-full overflow-hidden bg-muted"
                    >
                      {cert.image && (
                        <Image
                          src={cert.image}
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      ...
                      <div className="mt-auto pt-4 flex items-center justify-between">
                        <Link
                          href={`/certificates/${cert.id}`}
                          className="flex items-center text-xs font-semibold text-muted-foreground hover:text-foreground transition-colors group/link"
                        >
                          {t("viewDetails")}
                        ...
                        <a
                          ...
                          onClick={(e) =>
                            !cert.credentialUrl && e.preventDefault()
                          }
