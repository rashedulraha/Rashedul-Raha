...
export default function GuestbookGrid({ initialMessages }: { initialMessages: Record<string, string | number>[] }) {
  ...
  return (
    <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
      ...
      <div className="break-inside-avoid relative rounded-3xl overflow-hidden border border-white/12 shadow-[0_8px_32px_rgba(0,0,0,0.3)] bg-gradient-to-br from-white/8 to-white/2 backdrop-blur-xl group">
          ...
          {!isWriting ? (
            <>
              ...
              <button 
                ...
                className="relative z-20 flex items-center gap-2 px-6 py-2.5 rounded-full bg-white/5 border border-white/12 text-foreground text-sm font-medium hover:bg-white/10 hover:border-white/20 transition-all duration-300 backdrop-blur-md"
              >
            ...
            <form onSubmit={handleSubmit} className="w-full relative z-20 animate-in fade-in zoom-in duration-300 flex flex-col gap-3">
              <textarea 
                ...
                className="w-full px-4 py-3 bg-white/5 border border-white/12 rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50 resize-none h-24 text-sm transition-all"
              />
        ...
        <div className="h-20 bg-white/5 relative z-20 flex items-center justify-center gap-4 text-muted-foreground border-t border-white/12">
      ...
      {initialMessages.map((msg) => (
        <div key={msg.id} className="break-inside-avoid relative rounded-3xl overflow-hidden border border-white/12 shadow-[0_8px_32px_rgba(0,0,0,0.3)] bg-gradient-to-br from-white/8 to-white/2 backdrop-blur-xl group hover:-translate-y-1 hover:border-white/20 transition-all duration-300">
          ...
          <div className="h-20 bg-white/5 relative z-20 flex items-center justify-between px-6 border-t border-white/12">
            <div className="flex items-center gap-3">
              <div className={`flex items-center justify-center w-8 h-8 rounded-full ${msg.avatarBg} text-white font-bold text-xs uppercase shadow-lg border border-white/10`}>
