        ...
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-10 md:pt-40 md:gap-10">
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-xl md:text-5xl font-bold text-muted-foreground font-instrument-serif pl-20">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-bold text-muted-foreground font-instrument-serif">
                {item.title}
              </h3>
              {item.content}
            </div>
