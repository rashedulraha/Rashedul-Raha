...
function Sphere({ isDarkMode }: { isDarkMode: boolean }) {
  ...
  // Update uIsDark when theme changes
  useEffect(() => {
    if (materialRef.current) {
      materialRef.current.uniforms.uIsDark.value = isDarkMode ? 1.0 : 0.0;
    }
  }, [isDarkMode]);
  ...
  const fragmentShader = `
    ...
    void main() {
      // Mix between dark and light colors based on theme
      vec3 baseColor = mix(uColorLight, uColorDark, uIsDark);
      
      float intensity = 0.3 + vDisplacement * 2.0;
      vec3 color = baseColor * intensity;
      
      // In light mode, invert intensity for better visibility
      color = mix(baseColor * (1.0 - intensity * 0.5), color, uIsDark);
      
      float line = smoothstep(0.0, 0.02, abs(fract(vUv.x * 20.0) - 0.5));
      line *= smoothstep(0.0, 0.02, abs(fract(vUv.y * 20.0) - 0.5));
      
      // Adjust alpha based on theme for better contrast
      float alpha = mix(0.4, 0.6, uIsDark);
      
      gl_FragColor = vec4(color * (1.0 - line * 0.5), alpha);
    }
  `;
  ...
  if (!mounted) {
    return (
      <div className="w-full h-full flex items-center justify-center">
        <div className="w-64 h-64 rounded-full border border-border animate-pulse" />
      </div>
    );
  }
